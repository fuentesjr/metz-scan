...
module MetzScan
  ...
  class CheckPublishedGemTest < Minitest::Test
    ...
    def assert_exact_gem_pin(log)
      assert_includes log, %(gem_line=gem "metz-scan", "#{VERSION}")
    end
    ...
    def assert_external_temp_project(log, tmp_base)
      ...
      refute_match(/\A#{Regexp.escape("#{File.realpath(REPO_ROOT)}/")}/, install_pwd)
    ...
    def check_published_gem_path = File.join(REPO_ROOT, "bin/check_published_gem")
